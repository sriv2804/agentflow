...
@tool(name = "load_tool_group", description = "Load a tool group into your working context to access its tools")
async def load_tool_group(
    group_name : str,
    _ctx : ToolContext
):
    tool_grp = _ctx.tool_manager.get_tool_group(group_name)
    if tool_grp is None:
